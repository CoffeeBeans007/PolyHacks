...
df=pd.read_csv('Data/tot_ret.csv')
...
def fetch_historical_data(ticker_symbol, date):
...
# Update the 'Price' and 'Dividend' columns in the DataFrame using data from Alpha Vantage
for index, row in df.iterrows():
    date = row['Date'].strftime('%Y-%m-%d')
    price, dividend = fetch_historical_data('AAPL', date)
    if price is not None and dividend is not None:
        df.at[index, 'Price'] = float(price)
        df.at[index, 'Dividend'] = float(dividend)
