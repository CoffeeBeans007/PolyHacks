...
for column_name in data.columns:
    ...
    if data[column_name].isna().any():
        ...
        # Find the first row with NaN in the column
        first_nan_index = data[data[column_name].isna()].index.min()
        
        # Fetch downloaded_data starting from the first NaN row onward
        downloaded_data = downloaded_data.iloc[first_nan_index:]
        
        # Replace NaN values in the original DataFrame from the first NaN row onward
        data.loc[first_nan_index:, column_name] = downloaded_data
print(data)