...
for column_name in data.columns:
    ...
    if data[column_name].isna().any():
        ...
        first_nan = data[column_name].index[data[column_name].isna()][0]
         ...
         rows_to_fill = len(data) - (first_nan + len(downloaded_data))

        # Replace NaN values by fitting downloaded_data from the bottom to the top
        data[column_name].iloc[first_nan + rows_to_fill:first_nan + len(downloaded_data)] = downloaded_data.iloc[:-rows_to_fill].values
